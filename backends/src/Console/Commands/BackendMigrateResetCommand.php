...
class BackendMigrateResetCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'backend:migrate:reset';
     ...
     * @var string
     */
    protected $description = 'Rollback all database migrations for a specific or all backends';
     ...
     */
    public function __construct(Backends $backend, Filesystem $files, Migrator $migrator)
    {
        parent::__construct();
    ...
    protected function getMigrationPaths(){
        ...
        foreach ($this->getSlugsToReset() as $slug) {
            ...
            event($slug.'.backend.reset', [$this->backend, $this->option()]);
        }
    ...
    protected function getSlugsToReset()
    {
        ...
        if ($this->option("force")) {
            return $this->backend->all()->pluck("slug");
        }

        return $this->backend->enabled()->pluck("slug");
    }
    ...
    protected function validSlugProvided()
    {
        ...
        if ($this->backend->isEnabled($this->argument("slug"))) {
            return true;
        }
